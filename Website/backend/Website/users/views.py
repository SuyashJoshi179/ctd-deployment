...
class AccountDetailView(APIView):
    ...
    def get(self, request, **kwargs):
        ...
        orders = Order.objects.filter(user_id_fk=user.id)
        events = {}
        for order in orders:
            events[order.event_id_fk.event_name] = order.event_password
        data['events'] = events
            ...
            if events:
                for event in events:
                    with open('{}.csv'.format(event.event_name), 'w') as file:
                        ...
                        if objects:
                            writer.writerow(['username', 'password', 'senior'])
                            for obj in objects:
                                profile = Profile.objects.filter(user=obj.user_id_fk).first()
                                entry = ['{}'.format(obj.user_id_fk.username), '{}'.format(obj.event_password),
                                         '{}'.format(profile.senior)]
                                # print(entry)
